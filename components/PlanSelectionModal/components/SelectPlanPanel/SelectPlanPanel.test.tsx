...
describe("SelectPlanPanel", () => {
  it("shows a single card for the selected plan, with a Change Plan action", () => {
    ...
    expect(
      screen.getByRole("button", { name: /cambiar plan/i }),
    ).toBeInTheDocument();
  });
  ...
  it("shows the renewal-mode toggle for both monthly and annual billing", () => {
    ...
    expect(
      screen.getByRole("radiogroup", { name: /modo de renovación/i }),
    ).toBeInTheDocument();
  ...
  it("disables Continue while submitting", () => {
    ...
    expect(
      screen.getByRole("button", { name: /continuar|iniciando el pago/i }),
    ).toBeDisabled();
  });
});
