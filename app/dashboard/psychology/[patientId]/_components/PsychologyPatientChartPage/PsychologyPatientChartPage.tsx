...
import { getPatientPsychologyChart } from "@/app/actions/psychology";
import { PsychologyPatientChart } from "../PsychologyPatientChart";
import type { PsychologyPatientChartPageProps } from "./types";

export function PsychologyPatientChartPage({
  patientId,
}: PsychologyPatientChartPageProps) {
  const queryClient = useQueryClient();

  const { data, isLoading } = useQuery({
    queryKey: ["psychology-chart", patientId],
    queryFn: () => getPatientPsychologyChart(patientId),
  });
  ...
  return (
    <div className="p-6 space-y-6 max-w-4xl mx-auto">
      ...
      <PsychologyPatientChart
        patientId={chart.id}
        initialSessions={chart.sessions}
        initialGoals={chart.goals}
