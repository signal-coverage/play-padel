...
function makeRequest() {
  return new Request(
    "http://localhost/api/admin/clubs/club_1/tournaments/tourney_1/publish",
    { method: "POST" },
...
describe("POST /api/admin/clubs/[clubId]/tournaments/[tournamentId]/publish", () => {
  ...
  it("publishes the tournament for the route's clubId", async () => {
  ...
  it("returns 409 with the thrown error's message on conflict", async () => {
    findOwnedTournamentMock.mockResolvedValue({ id: "tourney_1" });
    publishTournamentMock.mockRejectedValue(new Error("Something went wrong"));
