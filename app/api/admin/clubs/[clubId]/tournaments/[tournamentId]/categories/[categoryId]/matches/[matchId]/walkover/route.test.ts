...
function makeRequest(body: unknown) {
  return new Request(
    "http://localhost/api/admin/clubs/club_1/tournaments/tourney_1/categories/cat_1/matches/match_1/walkover",
    { method: "POST", body: JSON.stringify(body) },
  ) as unknown as Parameters<typeof POST>[0];
}

const validBody = { winningTeamId: "team_1" };
...
describe("POST /api/admin/clubs/[clubId]/tournaments/[tournamentId]/categories/[categoryId]/matches/[matchId]/walkover", () => {
  ...
  it("records the walkover for the route's clubId and the admin's userId", async () => {
    findOwnedCategoryMock.mockResolvedValue({ id: "cat_1" });
  ...
  it("returns 409 with the thrown error's message on conflict", async () => {
    findOwnedCategoryMock.mockResolvedValue({ id: "cat_1" });
    recordWalkoverMock.mockRejectedValue(new Error("Something went wrong"));
