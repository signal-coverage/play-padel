...
import { recordWalkoverSchema } from "@/core/tournaments/schemas/tournament.schema";
...
// Admin-only mirror of POST .../matches/[matchId]/walkover.
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const parsed = recordWalkoverSchema.safeParse(body);
  if (!parsed.success) {
  ...
  try {
    await recordWalkover(
      matchId,
      parsed.data.winningTeamId,
      authResult.context.clubId,
    ...
    const message =
      err instanceof Error ? err.message : "Failed to record walkover";
