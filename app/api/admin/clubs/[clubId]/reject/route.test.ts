...
const rejectClubMock = rejectClub as ReturnType<typeof vi.fn>;
...
function makeRequest() {
  return new Request("http://localhost/api/admin/clubs/club_1/reject", {
    method: "POST",
  }) as unknown as Parameters<typeof POST>[0];
}

describe("POST /api/admin/clubs/[clubId]/reject", () => {
  beforeEach(() => {
    requireAdminMock.mockReset();
    rejectClubMock.mockReset();
  ...
  it("returns the auth failure response as-is when the caller is not an admin", async () => {
    ...
    expect(rejectClubMock).not.toHaveBeenCalled();
  });

  it("returns 404 when the club doesn't exist", async () => {
    rejectClubMock.mockResolvedValue({ status: "not_found" });
  ...
  it("returns 409 when the club is not currently PENDING", async () => {
    rejectClubMock.mockResolvedValue({ status: "not_pending" });
  ...
  it("rejects a PENDING club and logs an audit entry with the admin's own identity", async () => {
    rejectClubMock.mockResolvedValue({ status: "ok", clubId: "club_1" });

    const response = await POST(makeRequest(), makeParams("club_1"));

    expect(response.status).toBe(200);
    expect(rejectClubMock).toHaveBeenCalledWith("club_1");
