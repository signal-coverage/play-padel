...
function makeRequest() {
  return new Request(
    "http://localhost/api/clubs/tournaments/tourney_1/cancel",
    { method: "POST" },
...
beforeEach(() => {
  ...
  cancelTournamentMock.mockReset();
...
describe("POST /api/clubs/tournaments/[tournamentId]/cancel", () => {
  ...
  it("cancels the tournament when owned", async () => {
    requireClubOperationalMock.mockResolvedValue({ ok: true });
  ...
  it("returns 409 with the thrown error's message on conflict", async () => {
    requireClubOperationalMock.mockResolvedValue({ ok: true });
    findOwnedTournamentMock.mockResolvedValue({ id: "tourney_1" });
    cancelTournamentMock.mockRejectedValue(new Error("already cancelled"));
