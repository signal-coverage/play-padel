...
function makeRequest(date: string | null, signal?: AbortSignal) {
  const url = date
    ? `http://localhost/api/clubs/reservations/stream?date=${date}`
    : "http://localhost/api/clubs/reservations/stream";
...
describe("GET /api/clubs/reservations/stream", () => {
  beforeEach(() => {
    vi.useFakeTimers();
  ...
  it("returns the requireOwnerClub failure response as-is when the caller isn't an owner, without ever polling", async () => {
    ...
    const response = await GET(makeRequest("2026-09-10"));
  ...
  it("returns 400 when the date query param is missing or malformed, without ever polling", async () => {
    ...
    const malformed = await GET(makeRequest("not-a-date"));
  ...
  it("streams as text/event-stream and does not emit a changed event for the initial poll (it's just the baseline)", async () => {
    ...
    const response = await GET(makeRequest("2026-09-10", controller.signal));
  ...
  it("emits a changed event once the club's reservations for that date actually change (a new booking, a cancellation, or a status flip)", async () => {
