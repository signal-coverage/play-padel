 ...
 */
export async function getClubOperationalStatus(
  clubId: string,
): Promise<{ operational: boolean; cause: ClubOperationalCause | null }> {
  ...
  if (!mpConnected) {
    return { operational: false, cause: "MP_NOT_CONNECTED" };
  }

  const isActive = club?.status === "ACTIVE";
  if (!isActive) {
    return { operational: false, cause: "CLUB_INACTIVE" };
  }

  return { operational: true, cause: null };
}
